                    ...
                    {users.map((user) => (
                        <tr key={user.id} className="hover:bg-gray-50 transition">
                                ...
                                <span className={`px-2 py-1 rounded text-xs font-semibold
                                    ${user.role === 'ROLE_ADMIN' ? 'bg-purple-100 text-purple-800' :
                                        user.role === 'ROLE_STUDENT' ? 'bg-blue-100 text-blue-800' :
                                            user.role === 'ROLE_OIDB' ? 'bg-emerald-100 text-emerald-800' :
                                                user.role === 'ROLE_YGK' ? 'bg-amber-100 text-amber-800' :
                                                    user.role === 'ROLE_DEAN_OFFICE_STAFF' ? 'bg-rose-100 text-rose-800' :
                            ...
                            <td className="py-2 px-4 border-b text-sm text-gray-600">
                                {user.facultyName ? (
                                    <span className="text-amber-700 font-medium">{user.facultyName}</span>
                                ) : user.departmentName ? (
                                    <span className="text-teal-700 font-medium">{user.departmentName}</span>
                                ) : (
